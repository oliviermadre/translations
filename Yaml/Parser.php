 ...
 * @version    SVN: $Id: Parser.php 1568 2010-12-07 22:53:07Z olivier.madre $
 */
class Yaml_Parser
{
  protected
    ...
    while ($this->moveToNextLine())
    {
      ...
      if (preg_match('#^\-(\s+(?P<value>.+?))?\s*$#', $this->currentLine, $values))
      {
        ...
        else
        {
          ...
          {
            $data[] = array($matches[1] => Yaml_Inline::load($matches[2]));
          }
          else
          {
      ...
      {
        $key = Yaml_Inline::parseScalar($values['key']);
      ...
      else
      {
        ...
        if (1 == count(explode("\n", rtrim($this->value, "\n"))))
        {
          return Yaml_Inline::load($this->lines[0]);
        }
